				...
				<li><span> followers</span>
					<h3>
						<a href="#"> <i class="fa-heart"
							style="font-family: fontawesome; content: '\f004'; font-style: normal;"></i>
						</a> ${followers }
					</h3></li>
					...
					<c:choose>
						<c:when test="${chk eq 'follow' }">
							<button class="btn btn-primary"
								style="width: 350px; margin: 40px 0 20px 20px;"
								onclick="followFunction();">팔로우 하기</button>
						</c:when>
						<c:otherwise>
							<button id="follo" class="btn btn-primary"
								style="width: 350px; margin: 40px 0 20px 20px; background-color: red;"
								onclick="followFunction();">팔로우 취소</button>
						</c:otherwise>
					</c:choose>

					<button class="btn btn-primary"
						style="width: 350px; margin: 40px 0 20px 20px;"
						onclick="sendMsgFunction('${profile.u_name}');">쪽지 보내기</button>
				</c:if>
				<p class="bg-light" style="text-align: center;">skills</p>
			</div>
		</div>
	</div>
	<section>
